...
导入后可以先用内置的临时免费节点测试连通性；`自动测速` 会从全部节点里选择延迟最低的一个。长期使用建议在 QX 的节点资源里换成自己的节点订阅。

策略组命名尽量保持简单：`自动测速`、`香港`、`台湾`、`日本`、`新加坡`、`美国`、`proxy`、`Apple`、`Claude`、`Google`、`加密货币`、`Spotify`、`国际媒体`、`哔哩哔哩`、`兜底分流`。
